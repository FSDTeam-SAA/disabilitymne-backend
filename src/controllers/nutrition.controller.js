...
const normalizeNutrientsPer100g = (food, nutrients) => {
  const normalizedDataType = asString(food?.dataType).toLowerCase();
  const isBranded = normalizedDataType === "branded" || normalizedDataType.includes("branded");
  ...
  const servingGramWeight = convertToGrams(food?.servingSize, food?.servingSizeUnit);
  if (!servingGramWeight || servingGramWeight <= 0) {
    return nutrients;
  }

  const per100gFactor = 100 / servingGramWeight;
  const normalized = {};

  for (const [key, value] of Object.entries(nutrients || {})) {
    const parsed = Number(value);
    normalized[key] = Number.isFinite(parsed) ? round(parsed * per100gFactor, 4) : null;
  }
...
const mapFoodSummary = (food) => {
  ...
  const nutrientsPer100g = normalizeNutrientsPer100g(food, extractedNutrients);
  ...
  const display = {
    caloriesKcal:
      defaultPortionFactor > 0 && Number.isFinite(Number(nutrientsPer100g.caloriesKcal))
        ? round(Number(nutrientsPer100g.caloriesKcal) * defaultPortionFactor, 1)
        : null,
    proteinG:
      defaultPortionFactor > 0 && Number.isFinite(Number(nutrientsPer100g.proteinG))
        ? round(Number(nutrientsPer100g.proteinG) * defaultPortionFactor, 1)
        : null,
    carbsG:
      defaultPortionFactor > 0 && Number.isFinite(Number(nutrientsPer100g.carbsG))
        ? round(Number(nutrientsPer100g.carbsG) * defaultPortionFactor, 1)
        : null,
    fatG:
      defaultPortionFactor > 0 && Number.isFinite(Number(nutrientsPer100g.fatG))
        ? round(Number(nutrientsPer100g.fatG) * defaultPortionFactor, 1)
        : null,
  };
